...
function resetHighlight_concentration (event) {
   ...
   jsonLayer_concentration_camps.resetStyle(event.target);
};


function eventHandlerOnEachFeature_concentration(feature, layer) {
    ...
    layer.on({
         ...
         'mouseout': resetHighlight_concentration
     });
    ...
    layer.bindPopup(
      '<h3>'+'<strong>'+feature.properties.labelConvoyDeparturePlace+'</strong>'+'</h3>'+
      '<p>'+'<strong>'+feature.properties.quantity+'</strong>'+' persone dal nostro dataset sono state internate nel campo tra '+
      '<strong>'+feature.properties.labelConvoyDeparturePlaceDate_label+'</strong>'+' e '+'<strong>'+feature.properties.labelConvoyDeparturePlaceDateEnd_label
      +'</strong>'+'</p>'
    );
};

map.createPane('pane_jsonLayer_concentration_camps');
map.getPane('pane_jsonLayer_concentration_camps').style.zIndex = 403;
map.getPane('pane_jsonLayer_concentration_camps').style['mix-blend-mode'] = 'normal';

var jsonLayer_concentration_camps =  L.geoJSON(concentration_camps, {
  pane: 'pane_jsonLayer_concentration_camps',
  //set the style for the layer, using the function name present in layer_style.js
  style: style_concentration_camps,
  ...
  onEachFeature: eventHandlerOnEachFeature_concentration,
  ...
  pointToLayer: function (feature, latlng) {
    return L.circleMarker(latlng,{
     ...
     radius: (Math.log10(feature.properties.quantity)+1)*4
   })
  }
});
// We pass the variable jsonLayerPublications, created earlier, to the timeDimension method to manage geoJSON
var jsonLayer_concentration_camps_time =  L.TimeDimension.Layer.betweendates (jsonLayer_concentration_camps, {
...
jsonLayer_concentration_camps_time.addTo(map);
